#![allow(dead_code, reason = "support utilities used by unit tests")]
...
/// A lightweight in-process Nostr relay for unit tests.
///
        ...
        match cmd {
            "EVENT" => {
                if let Some(event_value) = arr.get(1) {
                    match crate::nostr_json::event_from_json(event_value.to_string()) {
                        Ok(event) => {
            ...
            "REQ" => {
                ...
                let filter: Filter =
                    match crate::nostr_json::filter_from_json(filter_value.to_string()) {
                        Ok(f) => f,
                        Err(_) => return,
                    };
