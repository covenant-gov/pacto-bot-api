...
- `agent.status`'s daemon-wide fields now include the running MDK version, the MLS wire generation, and the vendored OpenSSL/SQLCipher versions, so a future security advisory can be matched against a running deployment without reading the changelog. Handlers that have never registered MLS capability now receive a `bot_unavailable` event (rather than silence) naming which of two reasons applies — state lost to a reset and awaiting re-invitation, or engine unavailable after a fail-closed classification.
