...
- Migrated to `nostr` 0.44 and MDK (`mdk-core`/`mdk-sqlite-storage`) 0.8.0, matching the wire format `pacto-app` shipped in PR #198/#199. KeyPackage and Welcome events are now base64-encoded with an `encoding` tag per MIP-00/MIP-02, in both directions; a fetched KeyPackage or Welcome missing that tag is reported as a distinct peer-version-mismatch error (`-32025`) rather than a generic parse failure, so an operator can tell "peer is on the old build" apart from "genuinely malformed." Inbound KeyPackage fetches now accept both `kind:443` and `kind:30443`; the daemon continues to publish only `kind:443` (`pacto-app` fetches with a filter that already accepts both).
- **Bots must be re-invited after this upgrade.** The MLS store now requires a per-bot encryption key; on first start after upgrading, the daemon detects each bot's existing store, classifies it, and resets what it cannot open with the new key (see "Store reset and per-bot keys" below). A reset bot's groups are marked state-lost and reject sends with `-32026` until a group admin re-invites the bot; sends against a bot whose MLS engine could not be constructed after a fail-closed classification return `-32028`. `pacto-bot-admin diagnose` reports reset state, per-group state-lost/state-held status, and the sole-admin squads below, so an operator can work the list without reading logs. Follow the numbered upgrade procedure in [README.md's "Upgrading to nostr 0.44 / MDK 0.8.0"](README.md#upgrading-to-nostr-044--mdk-080) section.
- `admin.create_mls_group` and `agent.create_mls_group` accept an optional `admins` list (npub or hex Nostr public keys), additive-only; when omitted, the default admin set is now the creator plus the invited recipient instead of the creator alone, so a fresh group is never created sole-admin. Re-inviting an existing group member (e.g. after their own client reset) now removes then re-adds them within one `AddMember` command, publishing both evolution events in order so every peer stays in sync. New `admin.repair_mls_group_admins` JSON-RPC method and `pacto-bot-admin mls-group repair-admins --bot <id> --group <name>` CLI command expand an existing sole-admin group's admin set to every current member; refuses with the actual next step (restoration via re-invitation, or re-creation for a group the bot has no live state for) rather than a raw `GroupNotFound`.
- `pacto-bot-admin diagnose` splits pre-upgrade sole-admin squads into three buckets: repairable now (the bot still holds live state — run `mls-group repair-admins`), unrestorable (the bot was the only admin and no other member can act — re-create the group), and admin-set-unknown (the store was archived under an always-archived encrypted-store reset and its admin set cannot be harvested). The daemon's periodic tick also `warn!`s, once a configurable minimum age is reached, about groups still state-lost, bots whose MLS engine is still unavailable, and sole-admin squads still unrepaired, so a stuck bot shows up in logs instead of staying silently absent.
- `pacto-bot-admin export`/`import` carry the new per-bot MLS store key alongside the store. Export writes both into a caller-named, owner-only destination directory; import that can't use the key (missing or wrong) still imports the rest of the bot's state, marks every one of its groups state-lost, and reports what it skipped, rather than refusing the whole import.
- `agent.status`'s daemon-wide fields now include the daemon version and the MLS wire generation, broadcast to every registered handler on every status change. The fuller version matrix — daemon version, MDK version, MLS wire generation, and the vendored OpenSSL/SQLCipher versions — is reported by `pacto-bot-admin diagnose` (`version_info`), so a future security advisory can be matched against a running deployment without reading the changelog. Handlers that have never registered MLS capability now receive a `bot_unavailable` event (rather than silence) naming which of two reasons applies — state lost to a reset and awaiting re-invitation, or engine unavailable after a fail-closed classification.

### Security

- **Store reset and per-bot keys.** MDK 0.8.0 makes the MLS SQLCipher store's encryption key mandatory; the daemon now generates and holds a 32-byte key per bot, stored next to the store as `<store-filename>.key` (owner-only permissions from creation). Keeping the key beside the store is deliberate — an operator's `$DATA_DIR` backup captures both together — but it also means **the encryption buys nothing against anyone who can already read `$DATA_DIR`**; it is not a substitute for filesystem and backup access control. The MLS store, and any archive of a legacy pre-upgrade store, hold the plaintext content of every group message the bot decrypted, the same as the DM/attachment store already did — enabling `mls_archive_retention_days` retains message history and a backup agent will capture it. `mls_archive_retention_days` defaults to `0` (delete the legacy store immediately on reset, no archive); a store reset because its key was wrong or missing is always archived regardless of this setting, since that path cannot yet be told apart from operator error. A rollback to a pre-upgrade binary can invalidate messages a handler already acted on, because MDK enforces stricter forward-secrecy bounds than the store previously carried.
- `attachment::crypto::decrypt` zeroizes its working buffer when AES-GCM authentication fails. `decrypt_in_place_detached` decrypts before verifying the tag, so a forged or corrupted ciphertext previously left candidate plaintext in freed heap memory when the error propagated.

### Changed

- Windows (`x86_64-pc-windows-gnu`) and FreeBSD (`x86_64-unknown-freebsd`) are dropped from the release artifact matrix (`make package`, `.github/workflows/release.yml`) — [rusqlite#1025](https://github.com/rusqlite/rusqlite/issues/1025) blocks the Windows SQLCipher/crypto link, and [cargo-zigbuild#356](https://github.com/rust-cross/cargo-zigbuild/issues/356) blocks FreeBSD `kvm` symbol resolution. Both remain buildable manually (`make cross-compile-windows` / `make cross-compile-freebsd`); Windows also keeps a `cargo check` compile-only gate in CI so its code paths stay compiled.
...
- `pacto-bot-admin diagnose`, `doctor`, and `status`'s relay-connectivity checks (and any other caller of `check_relay_connectivity`) reported every `wss://` relay as unreachable with `"TLS support not compiled in"`, even when the daemon's own relay connection to the same URL succeeded. The health-check path used a direct `tokio-tungstenite` dependency with no TLS backend Cargo feature enabled, unrelated to the `rustls` crypto-provider fix above. Both `tokio-tungstenite` dependency declarations now enable `rustls-tls-webpki-roots`, matching the `rustls-tls` feature already used for `reqwest`.
- The `simulated_core_dump_after_nsec_load_does_not_leak_marker` secret-redaction test reported phantom leaks (`attachment_plaintext` on `main`, `attachment_key` on PR branches) because the scanner observed its own scratch memory. It allocated a snapshot buffer per writable region and read the region into it with `pread(2)` on `/proc/self/mem`; when the allocator handed back a buffer that lived *inside* the very region being snapshotted, the copy duplicated the region's bytes — including the fixture's own live markers — at addresses the exclusion list did not cover, and the self-overlapping copy re-emitted them a second time. The scan now reads through a single, never-resized scratch buffer whose address range is masked out of every chunk alongside the fixture markers, searches each region in place (with a carried overlap so a marker straddling a chunk boundary is still found) instead of concatenating every region into one multi-hundred-megabyte haystack, and uses `read_exact_at` so a short read no longer silently blanks the tail of a region. Verified against injected residue: a live unexcluded heap copy and freed heap residue are both still reported.
...
- Bot `id` values in `pacto-bot-api.toml` are now validated as slugs: lowercase letters, digits, hyphens, and underscores only, at most 64 characters, and must start/end with a letter or digit.
- The `picture` config field, if present, must be an `http://` or `https://` URL.
- Bot identities in `pacto-bot-api.toml` now require a unique `display_name` for deterministic alias-to-npub resolution. The daemon rejects configs with missing, empty, or duplicate display names at load time.
