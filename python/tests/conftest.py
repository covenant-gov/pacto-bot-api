...
Includes in-memory mock transport fixtures for unit tests and end-to-end
fixtures that spawn the Rust daemon, connect a JSON-RPC handler, and provide a
minimal WebSocket Nostr relay for contract tests.
"""
...
# ---------------------------------------------------------------------------
# End-to-end contract-test helpers
# ---------------------------------------------------------------------------
...
def discover_bot_files(examples_dir: Path | None = None) -> list[Path]:
    """Return every *_bot.py under python/examples/ except test files."""
    if examples_dir is not None:
        dirs = [examples_dir]
    else:
        dirs = [_repo_root() / "python" / "examples"]
    bots: list[Path] = []
